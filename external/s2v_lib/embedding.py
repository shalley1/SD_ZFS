#The embedmeanfield has been modified from the original version to return both the node embedding and graph embedding in forward. 
#original found at : https://github.com/Hanjun-Dai/pytorch_structure2vec/tree/master
...
class EmbedMeanField(nn.Module):
    ...
    def forward(self, graph_list, node_feat, edge_feat): 
        ...
        node_emb, graph_emb = self.mean_field(node_feat, edge_feat, n2n_sp, e2n_sp, subg_sp)
    ...
    def mean_field(self, node_feat, edge_feat, n2n_sp, e2n_sp, subg_sp):
        ...
        if self.output_dim > 0:
            ...
            node_embeddings = F.relu(out_linear)          # [N, D]
        else:
            node_embeddings = cur_message_layer           # [N, latent_dim]

        # Graph-level pooling (sum pooling as written)
        graph_embeddings = gnn_spmm(subg_sp, node_embeddings)  # [B, D]
